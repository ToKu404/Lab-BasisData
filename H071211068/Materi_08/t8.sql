...
-- Nomor 3
SELECT Kota,jumlah From (
(Select offices.city AS Kota , count(employees.firstname) as jumlah from Employees
JOIN offices
using (officecode)
where employees.firstName Like "L%" group by offices.city order by jumlah desc limit 1)
UNION 
(Select customers.city AS Kota , count(customers.customerName) as jumlah from customers
where customers.customerName Like "L%" group by customers.city order by jumlah desc limit 1))a ORDER BY jumlah desc limit 1;
