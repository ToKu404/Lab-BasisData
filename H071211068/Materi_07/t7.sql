...
FROM customers HAVING `Max Payment` IS NOT NULL;
...
-- nomor 4
select customers.customerName as 'Nama Pelanggan',concat(employees.firstName, " ", employees.lastName) AS 'Nama Karyawan'
, orders.orderNumber,SUM(od.priceEach*od.quantityOrdered) as 'Total Order' from employees 
join customers
...
(SELECT max(`Total Order`) FROM (SELECT SUM(orderdetails.priceEach*orderdetails.quantityOrdered) AS "Total Order"
...
FROM customers
GROUP BY country
HAVING CHAR_LENGTH(Country) in ((SELECT MAX(CHAR_LENGTH(Country)) FROM customers),(SELECT MIN(CHAR_LENGTH(Country)) FROM customers)) 
